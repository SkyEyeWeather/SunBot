...
def jsonToMeteoCourante(messageJson : dict) ->tuple:
  ...
  print(messageJson)
  #Création de l'embed :
  messageToSend = discord.Embed(title = "Météo actuelle sur {} [{}]".format(messageJson["name"], messageJson["sys"]["country"]), description=messageJson["weather"][0]["description"], color = 0x77b5fe)
  thumbnail = discord.File("./Data/Images/{}".format(dictWeatherCode[int(messageJson["weather"][0]["id"])]), "thumbnail.jpg")
  messageToSend.set_thumbnail(url="attachment://thumbnail.jpg")

  #Ajout des différents domaines :
  messageToSend.add_field(name="Température :", value=str(round(messageJson["main"]["temp"], 1)) + "°C", inline=True)
  messageToSend.add_field(name ="Température ressentie :", value=str(round(messageJson["main"]["feels_like"], 1)) + "°C", inline=True)
      ...
      if reponse.status_code != 200 :
        ...
        if reponseJson.get("alerts", None) == None:
          ...
          if self.decompteurStopAlerte <= 0 :
            ...
            alerteToSend.add_field(name="Début du phénomène :", value=datetime.fromtimestamp(reponseJson["alerts"][0]["start"]), inline=False)
            alerteToSend.add_field(name="Fin du phénomène :", value=datetime.fromtimestamp(reponseJson["alerts"][0]["end"]), inline=False)
            alerteToSend.add_field(name="Description :", value=reponseJson["alerts"][0]["description"], inline=False)
            #Envoie de l'embed sur les différents serveurs reliés au bot:
            for webhook in self.webhooksList:
              time.sleep(1)
              logoMeteoFrance = discord.File("./Data/Images/logoMeteoFrance.png", "logoMeteoFrance.png")
