...
def betaFunction(function):
  async def fonctionModifie(*args, **opt):
    if args[0].author.id != 691614947280551936:
      await args[0].channel.send("J'aimerais pouvoir t'aider, mais mon créateur (ce ravagé) me l'a interdit car cette fonctionnalité est actuellement en travaux !")
    else:
...
def adminFunction(function):
  async def fonctionModifie(*args, **opt):
    if args[0].author.id != 691614947280551936:
      await args[0].channel.send("Il faut être ravagé pour utilisé cette commande !")
    else:
      await function(*args, **opt)
  return fonctionModifie
...
async def on_ready():
  ...
  print("Création du dictionnaire des utilisateurs...")
  for member in sunBot.get_all_members():
    userBot = BotUser.BotUser(member)
    dictUsersBot[userBot.discordMember.id] = userBot
  print("Création du dictionnaire OK !")
  print("Mise en place des réactions...")
  dictUsersBot[690593377250443374].setEmoji("\U0001f31e", 0.25)  #Yaël
  dictUsersBot[305766040426643466].setEmoji("\U0001f329", 0.25)  #PY
  dictUsersBot[263549185645740032].setEmoji("\u2744", 0.25)      #Bastian
  dictUsersBot[516842042098319360].setEmoji("\U0001f32a", 0.25)  #Patrick
  print("Réactions OK")
  #Création du thread écoutant les alertes météos:
  webhookServeurTest1 = discord.Webhook.from_url('https://discord.com/api/webhooks/923863299270013018/6jfjT1QtrZ8UCXM1aEUhUD7z5G5Or9S3loFvlQs34Age8hX7VPfrD4UUQvGXCzmDN0Oo', adapter=discord.RequestsWebhookAdapter())
  ...
  webhookServeurCUPGE = discord.Webhook.from_url('https://discord.com/api/webhooks/921547043196002324/NJohjH9dduqidXHvV4Ei9V4KuIUvOiAPnbMEVPf_x06CUStZou0TlTapQi3B1i_zuLfp', adapter=discord.RequestsWebhookAdapter())
...
async def on_message(message):
  ...
  if messageMin in ["tête de pomme", "tete de pomme", "#tetedepomme"] :
    ...
    if messageTeteDePomme == 3:
      ...
      gifTeteDePomme = discord.File("./Data/TeteDePomme.gif", "TeteDePomme.gif")
...
async def meteo(ctx, nomLocalite="Toulouse"):
...
@sunBot.command(name="pluie", brief="Répond à la question existentielle : va t-il pleuvoir dans l'heure ? [en construction]")
async def pluie(ctx):
